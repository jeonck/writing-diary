...
코드블록을 완전히 비워두고 그날 실행이 돌면, `pipeline/generate.py`가 니체(Nietzsche)
명언 15개 중 그날 날짜에 해당하는 하나를 대신 사용해 일기를 생성한다. 포스트에는
"오늘의 명언"으로 표시되고 출처가 함께 붙는다. 명언은 15개를 날짜 기준으로 순환하며,
한 바퀴를 돌고 다시 나오는 명언은 새 문장을 적었을 때와 달리 매번 그날짜 기준으로
새로 게시된다(같은 명언이 반복돼도 매번 새 포스트가 생김).
