...
코드블록을 완전히 비워두고 그날 실행이 돌면, `pipeline/generate.py`가 니체, 마르쿠스
아우렐리우스, 세네카, 에픽테토스, 공자, 노자, 아리스토텔레스, 소크라테스, 에머슨,
소로우, 루미, 마크 트웨인, 헬렌 켈러, 벤자민 프랭클린, 레오나르도 다빈치, 부처 등
다양한 위인의 명언 풀(`FALLBACK_QUOTES`, `pipeline/generate.py`)에서 그날 날짜에
해당하는 하나를 대신 사용해 일기를 생성한다. 포스트에는 "오늘의 명언"으로 표시되고
저자가 함께 붙는다. 명언 풀을 날짜 기준으로 순환하며, 한 바퀴를 돌고 다시 나오는
명언은 새 문장을 적었을 때와 달리 매번 그날짜 기준으로 새로 게시된다(같은 명언이
반복돼도 매번 새 포스트가 생김). 명언을 더 추가/변경하고 싶으면 `FALLBACK_QUOTES`
리스트에 `{"text": ..., "author": ...}` 항목을 더하면 된다.
